...
import {
	CONNECTION_BUILDER_TEMPLATE,
	MODEL_CONNECTION_METHOD_TEMPLATE,
} from '@utils/helper-templates/connection-builder.template'
import {
	FILTER_BUILDER_TEMPLATE,
	MODEL_FILTER_METHOD_TEMPLATE,
} from '@utils/helper-templates/filter-builder.template'
import {
	SORT_BUILDER_TEMPLATE,
	MODEL_SORT_METHOD_TEMPLATE,
} from '@utils/helper-templates/sort-builder.template'
import {
	...
	private generateImports(context: HelperGenerationContext): string {
		const typeImports = context.models
			...
			.flatMap((model) => {
				...
				const imports = [
					graphqlName,
					`${graphqlName}QueryArgs`,
					`${graphqlName}Connection`,
					`${graphqlName}FilterInput`,
				]
				
				if (this.shouldGenerateSortForModel({ 
...
import { ${typeImports} } from './schema'
...
export interface ConnectionResult<T> {
	...
	private generateConnectionBuilder(helpers: ModelHelper[], context: HelperGenerationContext): string {
		const modelMethods = helpers
			.map((helper) => this.generateConnectionMethod(helper, context))
			.join('\n')
	...
	private generateConnectionMethod(helper: ModelHelper, context: HelperGenerationContext): string {
		...
		return MODEL_CONNECTION_METHOD_TEMPLATE
			.replace(/{{MODEL_NAME}}/g, helper.modelName)
	...
	private generateFilterBuilder(helpers: ModelHelper[], context: HelperGenerationContext): string {
		const modelMethods = helpers
			.map((helper) => this.generateFilterMethod(helper, context))
			.join('\n')
	...
	private generateFilterMethod(helper: ModelHelper, context: HelperGenerationContext): string {
		...
		return MODEL_FILTER_METHOD_TEMPLATE
			.replace(/{{MODEL_NAME}}/g, helper.modelName)
	}
	...
	private shouldGenerateFilterForModel(helper: ModelHelper, context: HelperGenerationContext): boolean {
		...
		return model.fields.some((field: any) => 
			context.attributeProcessor.model(model).field(field.name).isFilterable()
		)
	}

	private generateSortBuilder(helpers: ModelHelper[], context: HelperGenerationContext): string {
		const modelMethods = helpers
			.map((helper) => this.generateSortMethod(helper, context))
			.join('\n')
	...
	private generateSortMethod(helper: ModelHelper, context: HelperGenerationContext): string {
		...
		return MODEL_SORT_METHOD_TEMPLATE
			.replace(/{{MODEL_NAME}}/g, helper.modelName)
			.replace(/{{HAS_ID_FIELD}}/g, String(hasIdField))
	}
	...
	private shouldGenerateSortForModel(helper: ModelHelper, context: HelperGenerationContext): boolean {
		...
		return model.fields.some((field: any) => 
			context.attributeProcessor.model(model).field(field.name).isSortable()
		)
	}

	private generateFieldSelection(helpers: ModelHelper[]): string {
		const modelMethods = helpers
			.map((helper) => this.generateFieldSelectionMethod(helper))
			.join('\n')
	...
	private generateFieldSelectionMethod(helper: ModelHelper): string {
		...
		return MODEL_FIELD_SELECTION_METHOD_TEMPLATE
			.replace(/{{MODEL_NAME}}/g, helper.modelName)
			.replace(/{{RELATION_FIELDS}}/g, relationFields)
	}

	private generateIncludes(helpers: ModelHelper[]): string {
		const modelIncludes = helpers
			.map((helper) => this.generateModelInclude(helper))
			.join('\n')
	...
	private generateModelInclude(helper: ModelHelper): string {
		const relationIncludes = helper.relations
			.map((rel) => RELATION_INCLUDE_TEMPLATE.replace(/{{FIELD_NAME}}/g, rel.fieldName))
			.join(',\n\t')

		return MODEL_INCLUDE_TEMPLATE
			.replace(/{{MODEL_NAME_UPPER}}/g, helper.modelName.toUpperCase())
			.replace(/{{RELATION_INCLUDES}}/g, relationIncludes)
	}

	private findModelByGraphQLName(graphqlName: string, context: HelperGenerationContext): any {
		return context.models.find((model) => 
			context.attributeProcessor.model(model).name() === graphqlName
		)
	}
	...
	private modelHasIdField(model: any): boolean {
		...
		return model.fields.some((field: any) => 
			field.name === 'id' && !field.type.reference
		)
	}
